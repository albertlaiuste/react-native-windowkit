...
import { type WindowData, type WindowInteraction } from '@/types/windows';
import { type StackingContextValue } from '@/components/layouts/stacking/context';

type DragGestureParams<T extends WindowData> = {
  windowId: string;
  ...
  resizeGestures: Array<ReturnType<typeof Gesture.Pan>>;
  borderGestures: Array<ReturnType<typeof Gesture.Pan>>;
};

export function createDragGesture<T extends WindowData>({
  windowId,
  resolvedWindow,
  canvasSize,
  stackingContext,
  ...
  return Gesture.Pan()
    .requireExternalGestureToFail(...resizeGestures, ...borderGestures)
    .enabled(isUnlocked)
    .onBegin(() => {
      ...
      runOnJS(startInteraction)({ type: 'drag', id: windowId });
    })
    .onUpdate((event) => {
      const baseWindow =
